...
# ---- Helper used before (kept for reference; not needed once using validation) ----
def _pick_threshold_max_accuracy(y_true, scores, positive_label=-1):
    ...
    that maximizes accuracy on (y_true, scores). Uses test labels, so avoid for selection.
    """
    import numpy as np
...
# ---- NEW: threshold from validation to meet a target FP-rate on typicals ----
def _pick_threshold_by_fp_rate(scores_anom_val, fp_rate=0.05):
    """
    Choose anomaly threshold so that ~fp_rate of typical validation tiles are (falsely) flagged.
    'scores_anom_val' must be the anomaly score (higher = more anomalous) on typical-only validation.
    """
    s = np.asarray(scores_anom_val, dtype=float)
    s = s[np.isfinite(s)]
    if s.size == 0:
        return 0.0  # safe fallback
    q = np.clip(1.0 - float(fp_rate), 0.0, 1.0)
    return float(np.quantile(s, q))


# ------------------------------
#  Your pipeline functions below
# ------------------------------
...
def parafac_OC_SVM(rank, displayConfusionMatrix=False):
    ...
    # CP decomposition (train)
    decomposed_train = buildTensor(X_train, rank, num_train_sets, False)

    # Feature extraction + scaling (fit only on train)
    features_train = extractFeatures(decomposed_train, num_train_sets, False)
    scaler = StandardScaler()
    features_train_scaled = scaler.fit_transform(features_train)

    # -------- Validation data (typical-only) --------
    X_val, _ = readData(validation_data)
    num_val_sets = X_val.shape[0]
    decomposed_val = buildTensor(X_val, rank, num_val_sets, False)
    features_val = extractFeatures(decomposed_val, num_val_sets, False)
    ...
    # ---- Select best params on VALIDATION (typical-only) ----
    # Criterion: maximize (robust margin @ 5th pct, then acceptance rate) on validation normals
    best_score_tuple = (-np.inf, -np.inf)  # (robust_margin, accept_rate)
    best_model = None
    best_params = None
    ...
    for params in ParameterGrid(param_grid):
        ...
        preds_val = model.predict(features_val_scaled)  # +1 normal, -1 anomaly
        accept_rate = np.mean(preds_val == 1)  # higher is better on typical-only validation
        val_margins = model.decision_function(features_val_scaled).ravel()  # higher = more normal
        robust_margin = np.percentile(val_margins, 5)  # robust => higher is better

        score_tuple = (robust_margin, accept_rate)
        if score_tuple > best_score_tuple:
            best_score_tuple = score_tuple
            best_model = model
            best_params = params

    print(f"Chosen params via validation: {best_params} | "
          f"robust_margin={best_score_tuple[0]:.4f}, accept_rate={best_score_tuple[1]:.3f}")

    # ---- Test-time metrics (no tuning on test) ----
    # Continuous anomaly scores for ROC AUC (threshold-free)
    scores_normal_test = best_model.decision_function(features_test_scaled).ravel()
    scores_anom_test = -scores_normal_test  # higher = more anomalous
    auc_manual = manual_auc(true_labels, scores_anom_test, positive_label=-1)
    print("ROC AUC (manual, anomaly-positive):", auc_manual)

    # Accuracy at default cutoff (0) for reference (not used for selection)
    predictions_default = best_model.predict(features_test_scaled)  # +1 normal, -1 anomaly
    accuracy_default = np.mean(predictions_default == true_labels)
    print("Accuracy @ default cutoff (0):", float(accuracy_default))

    # ---- Threshold calibrated from validation (target FPR on typicals) ----
    scores_anom_val = -best_model.decision_function(features_val_scaled).ravel()
    target_fp = 0.05  # e.g., 5% FP on typical validation
    th_val = _pick_threshold_by_fp_rate(scores_anom_val, fp_rate=target_fp)
    y_pred_thresh = np.where(scores_anom_test >= th_val, -1, 1)
    accuracy_valcal = np.mean(y_pred_thresh == true_labels)
    print(f"Accuracy @ val-calibrated threshold (FPR~{target_fp:.0%} on val): {accuracy_valcal:.3f}")
    ...
    # Return test accuracy using the validation-calibrated threshold (signature unchanged)
    return accuracy_valcal
...
def ocsvm_raw_geography(displayConfusionMatrix=False):
    ...
    # Load data
    X_train, _ = readData(train_data)
    X_val, _   = readData(validation_data)       # typical-only validation
    X_test, true_labels = readData(test_data)
    true_labels = np.array(true_labels)
    ...
    for params in ParameterGrid(param_grid):
        ...
        preds_val = model.predict(X_val_scaled)  # +1 normal, -1 anomaly
        accept_rate = np.mean(preds_val == 1)
        margins = model.decision_function(X_val_scaled).ravel()  # higher = more normal
        robust_margin = np.percentile(margins, 5)
    ...
    # ---- Test-time metrics ----
    scores_normal_test = best_model.decision_function(X_test_scaled).ravel()
    scores_anom_test = -scores_normal_test  # higher = more anomalous

    auc_manual_ = manual_auc(true_labels, scores_anom_test, positive_label=-1)
    print("ROC AUC (manual, anomaly-positive):", auc_manual_)

    # Default 0 cutoff (reference only)
    predictions_default = best_model.predict(X_test_scaled)
    accuracy_default = np.mean(predictions_default == true_labels)
    print("Accuracy @ default cutoff (0):", float(accuracy_default))

    # Threshold from validation (target FP on typicals)
    scores_anom_val = -best_model.decision_function(X_val_scaled).ravel()
    th_val = _pick_threshold_by_fp_rate(scores_anom_val, fp_rate=0.05)
    y_pred_thresh = np.where(scores_anom_test >= th_val, -1, 1)
    ...
    acc_valcal = np.mean(y_pred_thresh == true_labels)
    print(f"Accuracy @ val-calibrated threshold: {acc_valcal:.3f}")
    return acc_valcal, best_params
...
def cp_rank_search_one_class_svm():
    """
    NOTE: This function still chooses rank by the returned test-set accuracy from parafac_OC_SVM,
    which now evaluates at a validation-calibrated threshold (less leakage than tuning by test).
    For fully leakage-free rank selection, consider computing a validation-only criterion per rank.
    """
...
# Driver (kept as in your code; assumes flags exist in your environment)
if enable_cp_oc_svm:
    if no_decomposition:
        one_class_svm()
    else:
